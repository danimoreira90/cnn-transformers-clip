# Plan: MBA Computer Vision â€” CNN, Transformers and CLIP
...
> Each task = one fresh subagent. Spec compliance review â†’ code quality review â†’ done.
...
## Phase 0 â€” Scaffold and rubric registry

Implements: the coverage guarantee behind every rubric line.

### Task 0.1 â€” Repo configuration
...
### Task 0.2 â€” Rubric registry
- **File**: `src/cv_mba/rubric.py`, `tests/test_rubric.py`, `evals/test_rubric_coverage.py`, `evidence.yaml`
- **Action**: Define `RUBRIC: dict[str, str]` holding all 26 ids R1.1â€“R1.5, R2.1â€“R2.5,
  R3.1â€“R3.6, R4.1â€“R4.4, R5.1â€“R5.6, each mapped to the rubric line in English. Define
  ...
  anchor.
- **TDD steps**:
  1. RED: `evals/test_rubric_coverage.py` asserts `missing_evidence(...) == []` against
...
- **Verification**: `uv run pytest -q` â†’ unit suite green. `uv run pytest evals/` â†’ fails
...
### Task 0.3 â€” Eval file and baseline
- **File**: `.claude/evals/cv-mba.md`
- **Action**: Write the eval file per `rules/edd-discipline.md` format with C1â€“C5 and
  R1â€“R4 as specified above. Record the BASELINE section: every capability eval fails
...
- **TDD steps**:
  1. RED: hand-build a 3Ã—3 confusion matrix with known values; assert recall per class
     matches hand-computed numbers; assert a class with zero support raises
     `ZeroSupportError` rather than returning NaN.
  2. GREEN: implement.
  3. Commit pair.
- **Verification**: `uv run pytest tests/test_metrics.py -v` â†’ all pass.
...
### Task 1.2 â€” ELPV loader and stratified split
...
- **TDD steps**:
  1. RED: assert index length 2,624; assert binary counts 1,803 functional and 821
     defective; assert the split preserves the labelÃ—type proportions within one
...
### Task 1.3 â€” A3 loader with the duplicate trap as an assertion
- **File**: `src/cv_mba/data.py`, `tests/test_data.py`
...
### Task 1.4 â€” COVID subsample and ADS-16 index
- **File**: `src/cv_mba/data.py`, `tests/test_data.py`
...
### Task 2.1 â€” Scaled dot-product attention
...
### Task 2.2 â€” Multi-head attention
- **File**: `src/cv_mba/attention.py`, `tests/test_attention.py`
...
### Task 2.3 â€” Transformer encoder block
...
### Task 2.4 â€” Positional encoding
...
- **TDD steps**:
  1. RED: encoding is added not concatenated, so sequence width is unchanged; and the
     central test â€” permute the token order, then assert the encoder output **changes**
...
- **Dependencies**: 2.4
- **Risk**: Medium â€” the attention-return path is where shape bugs hide.

### Task 2.6 â€” Determinism harness
...
- **TDD steps**:
  1. RED: two forward passes over the same batch produce bitwise-identical tensors. Mark
     `@pytest.mark.gpu`. When CUDA is absent, skip with an explicit reason string â€” never
...
### Task 3.2 â€” Feature extraction training
...
- **Exit criteria**: Eval C1 passes â€” the two partitions' top-5 lists differ by at least
  two entries. If they do not differ, that is a finding worth reporting, not a bug to hide.
- **Dependencies**: 2.6, 1.4
- **Risk**: Medium â€” the threshold must be justified in writing, and justifying a number
  chosen after seeing the results is the kind of reasoning the rubric is testing.

### Task 4.2 â€” Semantic search and the BERT tokenisation comparison
...
- **Action**: At least 8 queries spanning generic to specific and concrete to abstract,
  ...
  explicitly.
- **Verification**: Eval C2 â€” top-1 correct for the 4 concrete queries.
...
## Phase 5 â€” A1 notebook, Vision Transformers

Implements: F2.2, F3.2, F3.3. Rubric R2.2, R3.2, R3.3, R3.6.

### Task 5.1 â€” ViT from scratch on ELPV
...
- **Risk**: Medium â€” a from-scratch ViT on 2,100 training images is expected to perform
  poorly. That is the result, not a failure. Do not tune it into looking good; the
  comparison in 5.2 is the deliverable.

### Task 5.2 â€” Pretrained ViT fine-tune and comparison
- **File**: `notebooks/A1_vision_transformers.py`
- **Action**: Fine-tune a pretrained ViT on the same split, head replaced, same three
  ...
  spread.
- **Verification**: Eval C3 â€” `min delta > 0` across all three seeds and `mean delta >
  0.05`. Paste the per-seed table.
- **Exit criteria**: R3.3 and R3.6 evidence, with seed variance visible.
- **Dependencies**: 5.1
- **Risk**: Medium â€” ELPV is far from ImageNet, so the transfer benefit may be smaller
...
- **Risk**: Medium â€” exact numbers will not match the brief and are not expected to. The
  claim being reproduced is the pattern, not the digits.

### Task 6.2 â€” Conditional GAN
...
### Task 6.3 â€” Instability diagnosis and mitigation
- **File**: `notebooks/A4_estudo_caso_raio_x.py`
...
### Task 6.4 â€” Recall impact of synthetic augmentation
- **File**: `notebooks/A4_estudo_caso_raio_x.py`
...
- **Verification**: Eval C5 â€” `min recall delta > 0` across the three pairs. Paste all six
...
- **Dependencies**: 6.3
- **Risk**: Medium â€” synthetic augmentation may fail to help. If it does, that is a
  publishable result and the report says so. Do not re-roll seeds until a positive delta
  appears.

---

## Phase 7 â€” Report and submission

### Task 7.1 â€” Written rubric lines
...
- **Dependencies**: Phases 3â€“6
- **Risk**: Low

### Task 7.2 â€” AI usage citation
- **File**: `report/report.md`
...
- **Dependencies**: 7.1
- **Risk**: Low â€” and the cheapest possible way to lose the whole submission.

### Task 7.3 â€” Notebook headers from measured runs
...
### Task 7.4 â€” Close the rubric red
- **File**: `evidence.yaml`
- **Action**: Fill all 26 entries with real artefact paths and anchors.
- **Verification**: `uv run pytest evals/test_rubric_coverage.py -v` â†’ passes for the
...
- **Verification**: Eval R2 â€” `jupyter nbconvert --execute` completes for all four
...
- **Risk**: Medium â€” a notebook that runs in a warm session and fails from cold is the
