...
- **TDD steps**:
  1. RED: `evals/test_rubric_coverage.py` asserts `missing_evidence(...) == []` against
     the real `evidence.yaml`, and `tests/test_rubric.py` covers the checker's own logic.
     Run both. The eval must fail listing all 26 ids. Paste the output.
...
- **Action**: Generate `.ipynb` from the paired `.py` sources. Execute each one clean on
  Colab T4 top to bottom. Build the PDF. Assemble the ZIP as
  `daniel_moreira_deep-learning-and-vision_pd.ZIP` with a README.txt at its root naming
  the student, the discipline, the project and each file, so the inferred archive name is
  never the only thing identifying the submission.
